...
def process_data(raw_data, hide_zero=False, limit=None):
    df = pd.DataFrame(raw_data, columns = ["week_start", "total_km"])
    if df.empty:
        return df
...
def plot_running_volume(df):
    if df.empty:
        print("None running data was found")
        return
...
def main():
    ...
    df_processed = process_data(raw_data, hide_zero=args.hide_zero, limit=args.limit)
    plot_running_volume(df_processed)
